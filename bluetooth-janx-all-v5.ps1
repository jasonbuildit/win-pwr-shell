...
<#  bluetooth-janx-all-v5.ps1  (Windows PowerShell 5.1 x64)
    ...
    - Pair-BtByName convenience (first discoverable match)
    ...
    - Listen-ForKeyboard (e.g., Logitech) auto-pair
    - Listen-ForPairables (print Name + MAC for ANY device in pairing mode)
#>
  ...
  if ($PSCmdlet.ParameterSetName -eq 'ByName') {
    ...
    elseif ($NameOrId -match '^(secondary|second)$') { $Radio = ($adapters | Select-Object -Skip 1 | Select-Object -First 1)._raw }
      ...
      $match = $adapters | Where-Object {
        $_.Id -eq $NameOrId -or ($_.Name -and $_.Name -like "*$NameOrId*")
...
function Pair-BtByName {
  param([Parameter(Mandatory)][string]$NameMatch,[string]$Pin)
  $rx = New-Object System.Text.RegularExpressions.Regex ([regex]::Escape($NameMatch)), 'IgnoreCase'
  $c  = Find-BtDevices -Mode All | Where-Object { $_.Name -and $rx.IsMatch($_.Name) -and (-not $_.IsPaired) } | Select-Object -First 1
  if (-not $c) { throw "No discoverable device matching '$NameMatch'." }
  Write-Host "Attempting to pair: $($c.Name)" -ForegroundColor Cyan
...
function Remove-BtDevice {
  ...
  # 1) Try WinRT unpair first (no PnP required)
  try {
  ...
  # 3) Fallback to pnputil (works without PnP module)
  ...
  $psi.Verb = "runas"  # ensure elevation
...
# Pair first discoverable match by name (e.g., 'Logitech')
Pair-BtByName -NameMatch 'Logitech'   # optional: -Pin '0000'
