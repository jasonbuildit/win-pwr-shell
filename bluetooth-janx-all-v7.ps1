...
<#  bluetooth-janx-all-v7.ps1  (Windows PowerShell 5.1 x64)
    ...
    - Live BLE sniffer (Start-BleSniffer)
    ...
    - Listen-ForKeyboard: **now prints live advertisements** and pairs
    - Listen-ForPairables
#>
...
function Get-BtAdapterInfo {
  try {
    ...
    # Precompute values (PS 5.1 can't use inline "if" in hashtable values)
    $hasDefault = [bool]$adapter
    $addrHex    = $null
    if ($adapter -and $adapter.BluetoothAddress) { $addrHex = '{0:X12}' -f $adapter.BluetoothAddress }
...
function Start-BleSniffer {
  ...
  # Optional name filter made available to the event runspace
  if ($NameLike) {
  ...
  # Active scanning (gets more LocalName responses)
  ...
  # Helper (in script scope so the event can call it)
  ...
  $received = Register-ObjectEvent -InputObject $watcher -EventName Received -Action {
    try {
      ...
      if ($script:BleSniff_Count -gt $args.UserData) { return }  # respect MaxRows
  ...
  } -MessageData $null -UserData $MaxRows -PassThru

  try {
    $label = "no name filter"
    if ($NameLike) { $label = "filter NameLike='$NameLike'" }
...
function Listen-ForKeyboard {
  ...
  $snifferWatcher = $null; $snifferReceived = $null
  try {
    $snifferWatcher = [Windows.Devices.Bluetooth.Advertisement.BluetoothLEAdvertisementWatcher]::new()
    $scanModeType = [type]::GetType('Windows.Devices.Bluetooth.Advertisement.BluetoothLEScanningMode, Windows, ContentType=WindowsRuntime')
    if ($scanModeType) { $Active = [enum]::Parse($scanModeType,'Active'); try { $snifferWatcher.ScanningMode = $Active } catch { } }
    ...
    $snifferReceived = Register-ObjectEvent -InputObject $snifferWatcher -EventName Received -Action {
      try {
        $args = $EventArgs
        $mac  = ('{0:X12}' -f $args.BluetoothAddress) -replace '(.{2})(?=.)','$1:'
        $name = $args.Advertisement.LocalName
        if ($name -and $name.Length -gt 0) {
          ...
          $safeSvc = if ([string]::IsNullOrWhiteSpace($svc)) { '' } else { $svc }
          Write-Host ("ADV   MAC={0,-17} RSSI={1,4}dBm  Name={2}  Services=[{3}]" -f $mac, $args.RawSignalStrengthInDBm, $name, $safeSvc) -ForegroundColor Gray
        }
      } catch { }
    } -PassThru
  ...
  # Stop & clean sniffer
  try { if ($snifferWatcher -and $snifferWatcher.Status -ne ([Windows.Devices.Bluetooth.Advertisement.BluetoothLEAdvertisementWatcherStatus]::Stopped)) { $snifferWatcher.Stop() } } catch { }
  if ($snifferReceived) { Unregister-Event -SourceIdentifier $snifferReceived.Name | Out-Null }
