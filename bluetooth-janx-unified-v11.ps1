# bluetooth-janx-unified-v12.ps1
# PowerShell 7.5 host + Windows PowerShell 5.1 worker (WinRT Bluetooth).
# No .AsTask. JSON bridge returns structured objects to PS7.
# Features:
#   - Adapter info, list, toggle (PnP)
#   - Snapshot scan (BLE/Classic/All)
#   - Pair by DeviceInformation.Id or by Name (with retries & optional PIN)
#   - BLE sniffer (live ADV MAC/RSSI/Name/Services) in current console or a new PS5.1 window
#   - Listeners:
#       * Listen-ForKeyboard  (ADV + auto-pair loop)
#       * Listen-ForAnyDevice (ADV for everything + optional snapshot tables)
...
'@
  ...
  # Runner: convert PSCustomObject Args -> Hashtable inside PS5.1
  $runner = @"
...
try{
  ...
  $w.Start(); Start-Sleep -Seconds $sec; $w.Stop()
} finally { Unregister-Event -SourceIdentifier 'ADV' -ErrorAction SilentlyContinue | Out-Null }
'@
  $cmd = $cmd.Replace('{SEC}',$Seconds.ToString()).Replace('{FLT}', ($NameLike -replace '"','\"'))
...
function Listen-ForKeyboard {
  ...
  Start-Ps51SnifferWindow -Seconds $seconds -NameLike $NameMatch | Out-Null
...
function Listen-ForAnyDevice {
  ...
  $seconds = [int]([TimeSpan]::FromMinutes($TimeoutMinutes).TotalSeconds)
  Start-Ps51SnifferWindow -Seconds $seconds | Out-Null  # all devices, separate window
...
# ---------------- Quick help ----------------
Write-Host "Loaded bluetooth-janx-unified-v12.ps1 (PS7 bridge + 5.1 WinRT). Commands:" -ForegroundColor Cyan
@"
Get-BtAdapterInfo
Get-BtAdapters
...
Start-BleSniffer -Seconds 12 -NameLike 'Logi'           # sniffer in current console
Start-Ps51SnifferWindow -Seconds 20 -NameLike ''         # sniffer in separate window (all devices)
